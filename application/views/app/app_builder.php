...
if ($view_list) {
    ?>
    <form style='margin: auto;width: 980px;' action="<?= base_url() ?>
     app-landing-page/<?php echo $app_id; ?>" method="POST" class="full validate add_task_form"  id='app_view'/>
...
?>
<form style='margin: auto;width: 980px; position: relative;' action="<?= base_url() ?>
 app/edit/<?php echo $app_id; ?>" method="POST" class="full validate add_task_form"  
      enctype="multipart/form-data" id='app_edit'/>
<input type="hidden" id="is_edit" name="is_edit" value="0">
<table cellpadding="0" cellspacing="0" border="0" class="display" 
       id="example" style='width: 969px;box-shadow:0 0 6px rgba(0, 0, 0, 0.25);margin:15px 0 14px 0'>
    <tbody>
        <tr>
            <td style='width: 123px;'><strong>Application Name</strong></td>
            <td  style='width: 150px;'><input class="required textBoxLogin" type="text"
                                              name="app_name" id="d1_textfield" value="<?php echo $name; ?>" /></td>
            <td  style='width: 235px;'>
                <strong style="width: 110px; line-height: 20px; margin-top:
                               8px;">Application Icon (.png only)</strong>
...
//                if ($file_headers[0] == 'HTTP/1.1 200 OK') {
                if(file_exists
                   (FCPATH.'assets'.DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.'data'.
                    DIRECTORY_SEPARATOR.'form_icons'.DIRECTORY_SEPARATOR.$app_id.DIRECTORY_SEPARATOR.$icon)){
//                    echo $imagepathicon = FORM_IMG_DISPLAY_PATH . '../form_icons/' . $app_id . '/' . $icon;
                ...
                <input type="file" name="userfile" id="userfile"
                       accept="*.png" onchange="check_file()" style='display:none;'/>
            ...
            <td  style='width: 235px;'>
                ...
                if(file_exists(FCPATH.'assets'.DIRECTORY_SEPARATOR.
                               'images'.DIRECTORY_SEPARATOR.'data'.DIRECTORY_SEPARATOR.
                               'form_icons'.DIRECTORY_SEPARATOR.$app_id.DIRECTORY_SEPARATOR.$splash_icon)){
                ...
                <input type="file" name="splashfile" id="splashfile" accept="*.png"
                       onchange="check_file_splash(this)" style='display:none;'/>
        ...
        <tr>
            ...
            <td>
                ...
                <a id="create_application" onclick="$('#is_edit').val('1');
                      " style="top:-10px;" >Build New Version</a>
                <?php }?>
            </td>
        </tr>
            ...
            <td colspan="2" style="padding: 5px 0 5px 35px">
                <a href="<?= base_url() ?>assign-applicatioin-users/<?php echo $app_id?>"
                   style="background-color: green; padding: 5px; color: white; 
                          border-radius: 5px;text-decoration: none;"  class="cbp-tm-icon-archive">
                    Assign application to other users
                </a>
                <a target="new" href="<?= base_url() ?>web/test/<?php echo $app_id?>"
                   style="background-color: green; padding: 5px; color: white; 
                          border-radius: 5px; margin-left: 6px;text-decoration: none;
                          " class="cbp-tm-icon-archive">
                    Test in Web Mode
                </a>
            </td>
        </tr>
...
<div id="content" class="wrapper">
    <div id="controls-left">
        ...
        <ul id="form-fields">
            ...
            <li style='background: none;font-size: 16px;font-weight:
                       bold;cursor: default;'>Control Menu</li>
            <?php
            if(count($forms)==0)
            {
            ...
            <li id="form-icon-field" icon="<?php echo $form['icon']; ?>"
                icon-url="<?php echo FORM_IMG_DISPLAY_PATH . '../form_icons/' .
                    $app_id . '/' . $form['icon']; ?>" form-url="<?php echo $form['linkfile']; ?>"
...
//            print_r($app_settings);die;
            ...
            <select id="screen_size">
                ...
                if (isset($app_settings->screen_view) && 
                    $app_settings->screen_view == '1') {
                    echo 'selected';
                }
                ?>>Development Mode</option>
                <option value="2" <?php
                if (isset($app_settings->screen_view) &&
                    $app_settings->screen_view == '2') {
                    echo 'selected';
                }
                ?>>Sony Xperia Z</option>
                <option value="3" <?php
                if (isset($app_settings->screen_view) &&
                    $app_settings->screen_view == '3') {
                    echo 'selected';
                }
                ?>>HTC One</option>
                <option value="4" <?php
                if (isset($app_settings->screen_view) && 
                    $app_settings->screen_view == '4') {
                    echo 'selected';
                }
        ...
        <div class="mobile-image">
            <div id="form-builder" class="container">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="form-hd">
                            <form id="form-preview" action="" 
                                  class="TTWForm" method="post" 
                                  novalidate onSubmit="return submitform(this)">
    ...
    <div id="controls-right">
        <ul>
            ...
            if(count($forms)>1){?>
            <li class="li-checked" style="padding: 20px!important;
                                          height: 22px;">Control Menu</li>
            <?php }else{?>
                ...
                <li>
                    ...
                    if (!empty($form['icon'])) {
                        ...
                        <img  class="formIconsUpload" alt="" title="<?php echo $form['title']; ?>"
                             src="<?php echo FORM_IMG_DISPLAY_PATH . '../form_icons/' . $app_id . '/' . $form['icon'] ?>">
                        <?php
                    } ?>
                        <span title="<?php echo $form['title'];?>">
                            <?php
